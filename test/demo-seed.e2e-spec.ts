...
import { assertDemoSeedAllowed, runDemoSeed, DEMO_ADMIN, DEMO_METHODIST, DEMO_SUBJECT } from '../src/bootstrap/seed-demo';
import { cleanupAuthTables } from './test-db.helper';

const ADMIN_PW = 'DemoAdmin!123';
const METHODIST_PW = 'DemoMethodist!123';
const OK_ENV = { nodeEnv: 'test', allowDemoSeed: 'true', adminPassword: ADMIN_PW, methodistPassword: METHODIST_PW };
...
describe('Demo seed (e2e, izlan_test)', () => {
  ...
  it('DEMO-05 rerun is idempotent (no duplicate users / subject / assignments)', async () => {
    await runDemoSeed(deps(), OK_ENV);
    await runDemoSeed(deps(), OK_ENV);
    expect(await prisma.user.count({ where: { phone: { in: [DEMO_ADMIN.phone, DEMO_METHODIST.phone] } } })).toBe(2);
