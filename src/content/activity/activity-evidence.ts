 ...
 * LISTENING-COMPREHENSION. Answering a comprehension question about a visible text is READING-COMPREHENSION. Nothing
 * here emits 'free-production' — free/independent writing needs infrastructure we do not have, so it is never
 * fabricated (a point requiring it is simply unsatisfiable until that infra exists).
 *
...
export type EvidenceKind = 'recognition' | 'controlled-production' | 'listening-comprehension' | 'reading-comprehension';

/** Independence ordinal: 0 guided · 1 scaffolded/receptive (recognition, listening, reading) · 2 independent controlled-production. */
...
export const INDEPENDENCE_READING = 1;
...
export type ActivityInteractionKind = 'CHOICE' | 'STRUCTURED' | 'LISTENING' | 'READING';
 ...
 * structured → controlled-production@2; listening → listening-comprehension@1; reading → reading-comprehension@1.
 * Reading and recognition are BOTH independence 1 but are DISTINCT kinds: a reading gate cannot be satisfied by a
 * plain grammar-recognition activity (readiness checks the kind), and every measurement records its true kind.
 */
export function evidenceForActivity(kind: ActivityInteractionKind): ActivityEvidence {
  switch (kind) {
    ...
    case 'READING':
      return { evidenceKind: 'reading-comprehension', independenceLevel: INDEPENDENCE_READING };
  }
}
