...
export function RevisionMetadataForm({ editable, onReload }: { editable: boolean; onReload: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      else toast(describeError(e), 'error');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <Card className="space-y-3 p-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-muted">Versiya ma’lumotlari</h3>
        {editable && dirty && <Badge tone="warning">Saqlanmagan</Badge>}
      </div>
      {editable ? (
        <>
          <Field label="Sarlavha" htmlFor="rev-title">
            <Input id="rev-title" value={title} onChange={(e) => setTitle(e.target.value)} />
          </Field>
          <Field label="Tavsif" htmlFor="rev-desc">
            <Textarea id="rev-desc" rows={2} value={description} onChange={(e) => setDescription(e.target.value)} />
          </Field>
          <div className="flex items-center justify-between">
            <span className="text-xs text-muted">Davomiylik (server hisoblaydi): {revision.estimatedDurationMin ?? '—'} min</span>
        ...
        <div className="space-y-1 text-sm">
          ...
          <p className="text-xs text-muted">Davomiylik: {revision.estimatedDurationMin ?? '—'} min</p>
        </div>
      )}
    </Card>
  );
