  ...
  return (
    <div className="space-y-2">
      ...
      <ResourceView loading={res.loading} error={res.error} data={res.data} onRetry={res.reload}>
        {(skills) =>
          skills.length === 0 ? (
            <p className="text-xs text-muted">Ko‘nikma biriktirilmagan.</p>
          ) : (
              ...
              {skills.map((sk) => (
                <li key={sk.skillId}>
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-surface-2 px-2.5 py-1 text-xs text-text">
                    {sk.name}
                    {sk.status === 'ARCHIVED' && <Badge tone="danger">arxiv</Badge>}
                    {editable && (
                      <button type="button" aria-label={`${sk.name} ni olib tashlash`} onClick={() => remove(sk.skillId)} className="text-muted hover:text-danger">
                        <FiX aria-hidden />
                      </button>
                    )}
                  </span>
