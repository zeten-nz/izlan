...
export function MarkdownActivityEditor({
  ...
  const error = markdownValidationError(markdown);
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-3 lg:grid-cols-2">
        <Field label="Markdown matn" htmlFor="md" error={error}>
          <Textarea id="md" rows={8} value={markdown} onChange={(e) => setMarkdown(e.target.value)} className="font-mono" />
        </Field>
        {showPreview && (
          <div>
            <span className="mb-1.5 block text-sm font-medium text-text">Ko‘rinishi</span>
